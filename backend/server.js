...
const lineSeparatorRegex = new RegExp(`\\r\\n|\\n`);

let links = fs
  .readFileSync(linksFilePath, "utf-8")
...
async function fetchLink(link) {
  ...
  const handleLatencyData = async () => {
    ...
    try {
      if (!aborted) {
        // HEAD request
        const headResponse = await fetch(phplink + link.link, {
          method: "HEAD",
        });
...
app.post("/addLink", (req, res) => {
  ...
  const existingLinkIndex = links.findIndex(
    (existingLink) => existingLink.link === link,
  );
